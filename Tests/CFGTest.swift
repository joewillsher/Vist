...
func testExampleCFGOpt() throws -> Bool {
    ...
    let imem = try module.builder.build(AllocInst(memType: intType, irName: "i"))
    let i = try module.builder.build(IntLiteralInst(val: 0, size: 64))
    try module.builder.build(StoreInst(address: imem, value: i))
    
    let nmem = try module.builder.build(AllocInst(memType: intType, irName: "n"))
    let n = try module.builder.build(IntLiteralInst(val: 0, size: 64))
    try module.builder.build(StoreInst(address: nmem, value: n))
    ...
    do {
        ...
        let iterator = try module.builder.build(LoadInst(address: imem))
        let cmp = try module.builder.build(BuiltinInstCall(inst: .ieq, args: [iterator, one]))
    ...
    do {
        ...
        let two = try module.builder.build(IntLiteralInst(val: 2, size: 64))
        let n = try module.builder.build(LoadInst(address: nmem))
        let mod = try module.builder.build(BuiltinInstCall(inst: .irem, args: [n, two]))
        let zero = try module.builder.build(IntLiteralInst(val: 0, size: 64))
        let cmp = try module.builder.build(BuiltinInstCall(inst: .ieq, args: [mod, zero]))
        try module.builder.buildCondBreak(if: Operand(cmp),
    ...
    do {
        ...
        let val = try module.builder.build(LoadInst(address: nmem))
        let two = try module.builder.build(IntLiteralInst(val: 2, size: 64))
        let div = try module.builder.build(BuiltinInstCall(inst: .idiv, args: [val, two]))
        try module.builder.build(StoreInst(address: nmem, value: div))
    ...
    do {
        ...
        let mul = try module.builder.build(BuiltinInstCall(inst: .imul, args: [val, three]))
        let one = try module.builder.build(IntLiteralInst(val: 1, size: 64))
...
func testExampleCFGOpt2() throws -> Bool {
    ...
    let imem = try module.builder.build(AllocInst(memType: StdLib.intType.importedType(in: module), irName: "i"))
    let i = try module.builder.build(IntLiteralInst(val: 0, size: 64))
    let iaggr = try module.builder.build(StructInitInst(type: StdLib.intType, values: i, irName: "iaggr"))
    try module.builder.build(StoreInst(address: imem, value: iaggr))
    ...
    do {
        module.builder.insertPoint.block = b2
        let i2 = try module.builder.build(IntLiteralInst(val: 1, size: 64))
        let i2aggr = try module.builder.build(StructInitInst(type: StdLib.intType, values: i2, irName: "i2aggr"))
        try module.builder.build(StoreInst(address: imem, value: i2aggr))
        try module.builder.buildBreak(to: b4)
    }
    do {
        module.builder.insertPoint.block = b3
        let i3 = try module.builder.build(IntLiteralInst(val: 2, size: 64))
        let i3aggr = try module.builder.build(StructInitInst(type: StdLib.intType, values: i3, irName: "i3aggr"))
        try module.builder.build(StoreInst(address: imem, value: i3aggr))
        try module.builder.buildBreak(to: b4)
    }
    do {
        module.builder.insertPoint.block = b4
        let ptr = try module.builder.build(StructElementPtrInst(object: imem, property: "value"))
        let val = try module.builder.build(LoadInst(address: ptr))
        try module.builder.buildReturn(value: val)
    }
